...
float shadowCalc(vec4 fragPosLightSpace){
	vec3 projCoords = fs_in.fragPos.xyz / fragPosLightSpace.w;
...
{
	if(has_diffuse > 0){
		output_diffuse = vec3(texture(texture_diffuse1,fs_in.texCoords)).rgb;
	}else{
		output_diffuse = output_color;
	}

	if(has_specular > 0){
		output_specular = vec3(texture(texture_specular1,fs_in.texCoords)).rgb;
	}else{
	...
	for(int i=0;i<NR_POINT_LIGHTS; i++){
		result += calcPointLight(pLights[i], norm, fs_in.fragPos, cameraDir);
	}
